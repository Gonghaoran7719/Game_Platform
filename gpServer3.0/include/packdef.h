...
#define _DEF_SERVERIP /*"192.168.5.31"*/"10.50.243.24"  //TODO
...
typedef enum Net_PACK
{
    ...
    DEF_PACK_GAMEUP_RS,
    //上传
    DEF_PACK_GAMEUPUP_RQ,
    DEF_PACK_GAMEUPUP_RS,
...
//上传游戏发送包信息
typedef struct STRU_GAMEUP_RQ
{

STRU_GAMEUP_RQ()
    {
        m_nType =  DEF_PACK_GAMEUP_RQ;
        m_UserId = 0;
        filesize = 0;
 memset(m_name, 0 ,MAX_SIZE);
 memset(m_version, 0 ,MAX_SIZE);
 memset(m_os, 0 ,MAX_SIZE);
 memset(m_md5, 0 ,MAX_SIZE);
    }
    PackType m_nType; //包类型
    int m_UserId;    //用于 判断是不是1号管理员

    char m_name[MAX_SIZE];//文件名, 用于存储文件
    char m_version[MAX_SIZE];
    char m_os[MAX_SIZE];
    char m_md5[MAX_SIZE];//区分不同文件, 可采用 md5
    int filesize;//文件大小, 用于文件传输结束
}STRU_GAMEUP_RQ;

typedef struct STRU_GAMEUP_RS
{
...
//上传块请求
typedef struct STRU_GAMEUPUP_RQ
{
 STRU_GAMEUPUP_RQ()
    {
        m_nType =  DEF_PACK_GAMEUPUP_RQ;
        m_nUserId = 0;
        blocksize =0;
        memset(m_file,0,4096);
       memset(m_filename, 0 ,MAX_SIZE);
    }
    PackType m_nType; //包类型
    int m_nUserId; //用户 ID
   char m_filename[MAX_SIZE];  ///区分不同文件, 可采用 md5
    char m_file[4096];//块空间
    int64_t blocksize;//文件写入大小
}STRU_GAMEUPUP_RQ;


typedef struct STRU_GAMEUPUP_RS
{
    PackType   m_nType;   //包类型
    char result;
}STRU_GAMEUPUP_RS;
...
//删除游戏发送包
typedef struct STRU_GAMEDEL_RQ
{
    PackType   m_nType;   //包类型
...
typedef struct STRU_GAMEDEL_RS
{
    PackType   m_nType;   //包类型
    char result;
}STRU_GAMEDEL_RS;
...
//下载块回复包
typedef struct STRU_GAMEDOWN_RS
{
 STRU_GAMEDOWN_RS()
    {
        m_nType =  DEF_PACK_GAMEDOWN_RS;

        blocksize =0;
      memset(m_file , 0 ,MAX_SIZE);
       memset(m_filename, 0 ,MAX_SIZE);
    }
    PackType m_nType; //包类型
...
{
public:
    STRU_FILEINFO(): m_nFileSize(0),m_nPos(0),pFile(0)
    {
        memset(m_szFilePath, 0 , MAX_PATH);
        memset(m_szFileName, 0 , MAX_SIZE);
