...
static const ProtocolMap m_ProtocolMapEntries[] =
{
    ...
    {DEF_PACK_GAMEUP_RQ , &TcpKernel::UploadRq},
    {DEF_PACK_GAMEUPUP_RQ , &TcpKernel::UploadBlockRq},
    {DEF_PACK_GAMEDEL_RQ , &TcpKernel::delgame},
     {DEF_PACK_DOWNLOAD_RQ , &TcpKernel::downinfo},
    {0,0}
};
#define RootPath   "/home/lcl/exebage/"
...
int TcpKernel::Open()
{
    ...
    if(  !m_sql->ConnectMysql("localhost","root","032015","mygp")  )
    {
...
void TcpKernel::RegisterRq(int clientfd,char* szbuf,int nlen)
{
    ...
    STRU_REGISTER_RS rs;
    printf("clientfd:%d RegisterRq%s\n", rq->m_szPassword);


     char sqlBuff[_DEF_SQLIEN] ="";
     sprintf( sqlBuff , "select name from t_user where name= '%s';", rq->m_szPassword);
     printf("%s\n",sqlBuff);
    char sqlBuf[_DEF_SQLIEN] ="";
...
void TcpKernel::LoginRq(int clientfd ,char* szbuf,int nlen)
{
    ...
    STRU_LOGIN_RQ * rq = (STRU_LOGIN_RQ *)szbuf;
     printf("clientfd:%d RegisterRq%s\n", rq->m_szPassword);
     cout<<  rq->m_szPassword <<endl;
      cout<<  rq->m_szUser <<endl;
    STRU_LOGIN_RS rs;
//    //登录请求结果
...
void TcpKernel::UploadRq(int clientfd ,char* szbuf,int nlen)
{
    ...
    STRU_GAMEUP_RQ*rq= (STRU_GAMEUP_RQ *)szbuf;
         STRU_GAMEUP_RS rs;
         rs.m_nType=DEF_PACK_GAMEUP_RS;
         rs.result=false;

         FileInfo * info = new FileInfo;
         info->m_nPos = 0;

         strcpy( info->m_os, rq->m_os );
         strcpy( info->m_md5  , rq->m_md5);
         strcpy( info->m_szFileName, rq->m_name );
         strcpy( info->m_version  ,rq->m_version);
         sprintf( info->m_szFilePath    , "%s/%s" , RootPath ,   info->m_szFileName );
         info->m_nFileSize=rq->filesize;
        ...
        // select  mysql  is? exe
         char sqlBuf[_DEF_SQLIEN] ="";
            sprintf( sqlBuf ,  " select game_name from t_game where  game_md5='%s';",
                      rq->m_md5);
            list<string> res;
            m_sql->SelectMysql(sqlBuf, 1, res);
            if( res.size() > 0 )//miaochuan
            {
                rs.result=true;
                 m_tcp->SendData( clientfd , (char*)&rs , sizeof(rs) );
                 return ;
            ...
            m_mapFileToFileInfo[ info->m_szFileName ] =  info;//name map info
...
void TcpKernel::UploadBlockRq(int clientfd ,char* szbuf,int nlen)
{
    ...
    STRU_GAMEUPUP_RQ*rq= (STRU_GAMEUPUP_RQ *)szbuf;

    if( m_mapFileToFileInfo .find ( rq->m_filename ) == m_mapFileToFileInfo.end() ) return;

    FileInfo* info  = m_mapFileToFileInfo [ rq->m_filename ];

       //read or write the file "filename"


    int64_t res = fwrite( rq->m_file , rq->blocksize ,1 , info->pFile );
    info->m_nPos += res;

    cout<<"weizhi"<< info->m_nPos <<endl;

    if(  rq->blocksize < MAX_CONTENT_LEN || info->m_nPos >= info->m_nFileSize  )
    {
        //写完了
        fclose( info->pFile);
                  ...
                  char sqlBuf[_DEF_SQLIEN] ="";
                     sprintf( sqlBuf , "insert into t_game(game_name,game_version,game_os,game_md5,game_downcount,game_path) values('%s','%s','%s','%s',0,'%s');",
                              info->m_szFileName,info->m_version,info->m_os,info->m_md5,info->m_szFilePath);


            printf("%s\n" , sqlBuf);
            if( !m_sql->UpdataMysql(sqlBuf ) )
            {
                cout<< "UpdataMysql erro :" << sqlBuf <<endl;
            }
            //返回

            STRU_GAMEUPUP_RS  rs;

            rs. m_nType=DEF_PACK_GAMEUPUP_RS;   //包类型

            rs.result = 1;
            m_tcp->SendData( clientfd , (char*)&rs , sizeof(rs));
            m_mapFileToFileInfo.erase( rq->m_filename );
           delete info;
           info = NULL;
    }
...
void TcpKernel::delgame(int clientfd, char *szbuf, int nlen)

{
      STRU_GAMEDEL_RQ*rq= (STRU_GAMEDEL_RQ *)szbuf;
        STRU_GAMEDEL_RS rs;
         rs.m_nType=DEF_PACK_GAMEDEL_RS;   //包类型
         rs.result=false;
           char sqlBuf[_DEF_SQLIEN] ="";
              sprintf( sqlBuf ,  " select game_path from t_game where  game_name='%s';",
                        rq->m_name);
              cout<< rq->m_name<<endl;
              list<string> res;
              m_sql->SelectMysql(sqlBuf, 1, res);
              if( res.size() > 0 )
              {
                  ...
                  if( std::remove( res.front().c_str()) == 0 )
                  {
                          cout<<"删除成功"<<endl;
                      }
                      else
                  ...
                  char sqlBuff[_DEF_SQLIEN] ="";
                     sprintf( sqlBuff,  "delete from t_game where game_name='%s';",rq->m_name );
                     if( !m_sql->UpdataMysql(sqlBuff ) )
                     {
                         cout<< "UpdataMysql erro :" << sqlBuff <<endl;
                     }
...
//  down  deal
 void TcpKernel:: downinfo(int clientfd, char *szbuf, int nlen)
  {
      ...
      STRU_DOWNLOAD_RS rs;
              char sqlBuf[_DEF_SQLIEN] ="";
                 ...
                 if( res.size() > 0 )
                 {
                     ...
                     strcpy( rs.m_name,rq->m_name);
                      FILE *fp=fopen(res.front().c_str(),"r");
                        if(!fp) return  ;
                        fseek(fp,0L,SEEK_END);
                        rs.m_nFileSize=ftell(fp);
                        fclose(fp);
                       m_tcp->SendData( clientfd , (char*)&rs , sizeof(rs));
                 }
              ...
              //send block
                 ...
                 if( fp )
                 {
                     while(1)
                     {
                         STRU_GAMEDOWN_RS rrs;
                         rrs.m_nType=DEF_PACK_GAMEDOWN_RS;
                         int64_t res = fread(  rrs.m_file,1 ,  MAX_CONTENT_LEN ,fp );
                         rrs.blocksize=res;
                                 strcpy(rrs.m_filename,rs.m_name);
                            ress+= res;
                            cout<<"------------------"<<endl;
                            cout<<ress<<endl;
                            cout<<rs.m_nFileSize<<endl;
                         m_tcp->SendData( clientfd , (char*)&rrs , sizeof(rrs) );

                          if( ress >=  rs.m_nFileSize)
                          {
